<?php # Script 17.5 - browse_tablets.php
// This page displays the available tablets.

session_start(); // Start the session.

// Set the page title and include the HTML header:
$page_title = 'Best Deal | Tablets';
...
$q = "	SELECT brands.brand_id, products.product_id, brands.brand_name AS Κατασκευαστής, products.product_name AS Μοντέλο, products.description AS Περιγραφή, products.price AS Τιμή FROM products, categories, brands
		WHERE products.category_id=categories.category_id AND products.brand_id=brands.brand_id AND categories.category_name='Tablets'
...
if (isset($_GET['bid']) && is_numeric($_GET['bid']) ) {
	...
	if ($bid > 0) { // Overwrite the query:
		$q = "	SELECT brands.brand_id, products.product_id, brands.brand_name AS Κατασκευαστής, products.product_name AS Μοντέλο, products.description AS Περιγραφή, products.price AS Τιμή FROM products, categories, brands
				WHERE products.category_id=categories.category_id AND products.brand_id=brands.brand_id AND categories.category_name='Tablets' AND brands.brand_id = $bid
				ORDER BY products.price ASC";
	}
}
...
while ($row = mysqli_fetch_array ($r, MYSQLI_ASSOC)) {
	...
	// Display each record:
	echo "\t<tr>
		<td align=\"center\"><a href=\"browse_tablets.php?bid={$row['brand_id']}\">{$row['Κατασκευαστής']}</a></td>
